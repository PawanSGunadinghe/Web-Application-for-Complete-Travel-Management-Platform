// src/Pages/Admin/BookingManager/CustomeBookDetails.jsx
import React, { useEffect, useMemo, useState } from "react";
import { Link } from "react-router-dom";
import IMG1 from "../../../Assets/lk.jpg";

export default function CustomeBookDetails() {
    ...
    return (
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10">
            ...
            <Link
                ...
                className="inline-flex items-center gap-2 rounded-full bg-blue-600 px-4 py-2 text-white text-sm font-medium shadow hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                aria-label="Add your package"
            >
                ...
                <span>Add your package</span>
            </Link>
            </div>
        </div>

        {/* Body */}
        <main className="max-w-7xl mx-auto px-6 md:px-10 pb-12">
            {err && (
            ...
            {loading
                ? Array.from({ length: 8 }).map((_, i) => (
                    <li
                    key={i}
                    className="relative rounded-xl bg-white border border-slate-200 p-5 animate-pulse"
                    >
    ...
    function Card({ req }) {
    const { _id, fullName, country, travellers, preferredDates, createdAt } = req || {};
    const dateRange = formatDateRange(preferredDates?.start, preferredDates?.end);

    return (
        <div>
        <Link
            to={`/custom-request/${_id}`}
        ...
        >
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
                ...
                <Pill>
                {travellers ? `${travellers} traveller${travellers > 1 ? "s" : ""}` : "—"}
                </Pill>
            </div>
