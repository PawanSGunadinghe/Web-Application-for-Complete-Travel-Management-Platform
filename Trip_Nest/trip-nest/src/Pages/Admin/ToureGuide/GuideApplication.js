...
export default function GuideApplicationForm() {
  // const navigate = useNavigate(); // optional
    ...
    const handleChange = (e) => {
        ...
        setForm((f) => ({ ...f, fullName: lettersOnly }));
        return;
        }

        // address: limit to 250 characters
        if (name === "address") {
        const limitedValue = value.slice(0, 250);
        setForm((f) => ({ ...f, address: limitedValue }));
        setErrors((err) => ({
            ...err,
        ...
        // languages: only letters, commas, and spaces
        if (name === "languages") {
        const lettersOnly = value.replace(/[^A-Za-z\s,]/g, '');
        setForm((f) => ({ ...f, languages: lettersOnly }));
        return;
        }

        // education: limit to 500 characters
        if (name === "education") {
        const limitedValue = value.slice(0, 500);
        setForm((f) => ({ ...f, education: limitedValue }));
        setErrors((err) => ({
            ...err,
            education: limitedValue.length > 500 ? "Education must be 500 characters or less" : "",
        }));
        return;
        }

        // national ID/Passport: only numbers, V/v, and N, max 12 characters
        if (name === "nationalIdOrPassport") {
        const validChars = value.replace(/[^0-9VNvn]/g, '').slice(0, 12);
        setForm((f) => ({ ...f, nationalIdOrPassport: validChars }));
        setErrors((err) => ({
            ...err,
            nationalIdOrPassport: validChars.length > 12 ? "Must be 12 characters or less" : "",
        }));
        return;
        }

        // emergency contact name: only letters and spaces
        if (name === "emergencyContactName") {
        const lettersOnly = value.replace(/[^A-Za-z\s]/g, '');
        setForm((f) => ({ ...f, emergencyContactName: lettersOnly }));
        return;
        }
        ...
        if (name === "birthday") {
        setErrors((err) => ({ ...err, birthday: isAgeValid(value) ? "" : "Must be 21-60 years old" }));
        }
        if (name === "nationalIdOrPassport") {
    ...
    const validateAll = () => {
        ...
        else if (!/^[A-Za-z\s]+$/.test(form.fullName)) next.fullName = "Only letters and spaces allowed";

        if (!form.birthday) next.birthday = "Required";
        else if (!isAgeValid(form.birthday)) next.birthday = "Must be 21-60 years old";
    ...
    return (
        ...
        <form onSubmit={handleSubmit} className="max-w-5xl mx-auto px-6 md:px-10 py-10 space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Field label="Birthday" error={errors.birthday} hint="Must be 21-60 years old">
                <input
                    type="date"
                    name="birthday"
                    value={form.birthday}
                ...
                <textarea
                    ...
                    rows={3}
                    maxLength={250}
                    className={`w-full rounded-md bg-gray-100 border px-4 py-2.5 focus:outline-none focus:ring-2 resize-y ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <input
                    ...
                    onChange={handleChange}
                    className={`w-full rounded-md bg-gray-100 border px-4 py-2.5 focus:outline-none focus:ring-2 ${
                        errors.languages ? "border-red-500 focus:ring-red-300" : "border-gray-200 focus:ring-gray-300"
                    }`}
                ...
                <Field label="Educational Qualifications" error={errors.education} full>
                <textarea
                    name="education"
                    ...
                    maxLength={500}
                    className={`w-full rounded-md bg-gray-100 border px-4 py-2.5 focus:outline-none focus:ring-2 resize-y ${
                        errors.education ? "border-red-500 focus:ring-red-300" : "border-gray-200 focus:ring-gray-300"
                    }`}
                    placeholder="Diploma/Certificate, Institution, Year"
                />
                <div className="flex justify-between items-center mt-1">
                    <p className="text-xs text-gray-500">Maximum 500 characters</p>
                    <p className={`text-xs ${form.education.length > 500 ? "text-red-600" : "text-gray-500"}`}>
                        {form.education.length}/500
                    </p>
                </div>
                </Field>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                />
                <div className="flex justify-between items-center mt-1">
                    <p className="text-xs text-gray-500">Only numbers, V/v, and N allowed</p>
                    <p className={`text-xs ${form.nationalIdOrPassport.length > 12 ? "text-red-600" : "text-gray-500"}`}>
                        {form.nationalIdOrPassport.length}/12
                    </p>
                </div>
                </Field>
    ...
    function isAgeValid(yyyyMmDd) {
    if (!yyyyMmDd) return false;
    const b = new Date(yyyyMmDd);
    if (isNaN(b)) return false;
    const now = new Date();
    const minAge = new Date(now.getFullYear() - 60, now.getMonth(), now.getDate());
